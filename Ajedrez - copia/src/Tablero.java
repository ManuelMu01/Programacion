...
public class Tablero  {
	...
	public Tablero() {
		...
		casillas[2][2]=new NuevaPieza(2, 2, 'N');
		casillas[5][2]=new NuevaPieza(5, 2, 'N');
		...
		/* mirar tablas
		casillas[4][0]=new Rey(4, 0, 'N');
		casillas[3][1]=new Torre(3, 1, 'B');
		casillas[5][1]=new Torre(5, 1, 'B');
		casillas[4][2]=new Alfil(4, 2, 'B');
		casillas[0][7]=new Torre(0, 7, 'N');
		casillas[7][7]=new Caballo(7, 7, 'B');
		casillas[7][6]=new Rey(7, 6, 'B');
		*/
	}
...
//							T.mover(new Posicion(x,y), posficha);
	...
	public void convertirReina (Tablero T) {
		...
		for (int x=0;x<8;x++) {
			if(T.casillas[x][0]!= null &&T.casillas[x][0].forma.equals("\u265F")) {
				T.casillas[x][0]=new Reina(x, 0, 'B');
		...
		for (int x=0;x<8;x++) {
			...
			if(T.casillas[x][7]!= null && T.casillas[x][7].forma.equals("\u2659")) {
				T.casillas[x][7]=new Reina(x, 7, 'N');
				System.out.println("llegue");
			}	
		}
...
//						T.mover(new Posicion(x,y), posficha);
						...
						boolean jaque=false;
						// miramos en el tablero puedo mover mi rey sin que se muera
						for (Posicion posRey : T.casillas[xRey][yRey].posiblesPosiciones(T)) {
							Ficha ff=T.casillas[posRey.x][posRey.y];
							
							casillas[posRey.x][posRey.y]=casillas[xRey][yRey];
							casillas[posRey.x][posRey.y].posicion=new Posicion(posficha.x,posficha.y);
							casillas[xRey][yRey]=null;
							
							// miro si muere
							for (int xx = 0 ; xx < 8 ; xx++) {
								for (int yy = 0 ; yy < 8 ; yy++) {
									// miramos un ficha suya y miramos si hace jaque algun posible movimiento
									if (casillas[xx][yy] != null && casillas[xx][yy].color!=T.tirada) {
										for (Posicion possficha : T.casillas[xx][yy].posiblesPosiciones(T)) {
											if (possficha.x==xRey && possficha.y==yRey) jaque=true;
										}
							...
							casillas[xRey][yRey]=casillas[posRey.x][posRey.y];
							casillas[xRey][yRey].posicion=new Posicion(xRey,yRey);
							casillas[posRey.x][posRey.y]=ff;
						}
